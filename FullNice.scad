...
// Общая глубина зеленой рамки (без ребра!)
green_frame_total_depth = front_wall_thickness + inner_frame_depth + yellow_plate_thickness + back_wall_thickness;

// Внешняя ширина
outer_x_w = fixture_width + (front_wall_thickness * 2);

// Положение желтой пластины (начало - в конце полости)
...
// Координата для фиксирующих блоков (там же, где желтая пластина)
...
// ═══════════════════════════════════════════════════════════════════
// ЗЕЛЕНАЯ РАМКА (без ребра, цельная задняя стенка 1.6мм, пирамидки у передней стенки)
// ═══════════════════════════════════════════════════════════════════

module green_frame(pd, pcl, pth, ht, xw, yd) {
    ...
    union() {
        ...
        // Левый фиксирующий блок (у начала желтой пластины)
        ...
        // Правый фиксирующий блок (у начала желтой пластины)
        ...
        // ЦЕЛЬНАЯ ЗАДНЯЯ СТЕНКА (после желтой пластины)
...
if (view_mode == "all") {
    ...
    green_y_d = green_frame_total_depth;
    ...
    // СТАНЦИЯ 2: Желтая пластина с шариками
    translate([300, 0, 0]) 
        color("Yellow") plate_with_spheres(p_d, p_cl, p_th, h_t, true, cup_shape);

    // СТАНЦИЯ 3: Зеленая рамка
    translate([600, 0, 0]) 
        color("RoyalBlue") green_frame(p_d, p_cl, p_th, h_t, outer_x_w, green_y_d);
    
    // СТАНЦИЯ 4: Синяя нижняя форма (4 гнезда под пирамидки - спереди и сзади)
...
// ═══════════════════════════════════════════════════════════════════
// ВСПОМОГАТЕЛЬНЫЕ МОДУЛИ ДЛЯ ОСНАСТКИ (ЛАСТОЧКИН ХВОСТ)
// ═══════════════════════════════════════════════════════════════════
...
translate([-800, 0, 0]) {
    difference() {
        ...
        translate([0, 0, h_t/2]) rotate([0, 0, 180]) 
            dovetail_rail_vertical(width = 15 + dovetail_gap, depth = 10, height = h_t + 12, angle = 20);
    }
}
...
translate([-450, 0, 0]) {
    color("LightGreen") standalone_plate_only(p_d, p_cl, yellow_plate_thickness, h_t, true);
    translate([fixture_width/2, 0, h_t/2]) color("LightGreen")
        dovetail_rail_vertical(width = 15, depth = 10, height = h_t, angle = 20);
}
