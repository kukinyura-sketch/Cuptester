...
// МОДИФИКАЦИЯ: Выступающая мастер-пластина + бортик опалубки + автоматический подбор размеров + режим сборки + расчёт гипса

// ═══════════════════════════════════════════════════════════════════
// 1. РЕЖИМЫ ОТОБРАЖЕНИЯ
// ═══════════════════════════════════════════════════════════════════

view_mode = "all"; // [catalog:Каталог форм, mug:Готовая кружка, master:Мастер-модель, all:Вся оснастка, assembly:СБОРКА В ОПАЛУБКЕ]
...
show_spheres = true;            // [true:ПОКАЗАТЬ СФЕРЫ, false:СКРЫТЬ СФЕРЫ]
show_calibration_points = true; // [true:ПОКАЗАТЬ ТОЧКИ, false:СКРЫТЬ ТОЧКИ]
...
formwork_height_preset = 150; // [100:Малая (100мм), 150:Средняя (150мм), 200:Большая (200мм)]
...
pyramids_y = pyramid_offset + pyramid_size/2;
...
function get_inner_volume() = 
    ...
    echo("DEBUG outer_radius: ", outer_radius, " inner_radius: ", inner_radius, " inner_height: ", inner_height)
...
gypsum_base = 900;  // базовое количество гипса для таблицы
...
module formwork(pd, pcl, pth, ht, xw, yd) {
    ...
    union() {
        ...
        // Левая пирамидка
        translate([pyramid_offset + pyramid_size/2, pyramids_y, 0])
            truncated_pyramid(pyramid_size, top_size, pyramid_height);
        
        // Правая пирамидка
        translate([fixture_width - pyramid_offset - pyramid_size/2, pyramids_y, 0])
            truncated_pyramid(pyramid_size, top_size, pyramid_height);
    }
}
...
if (view_mode == "all") {
    ...
    translate([900, -total_depth/2, 0]) 
        color("DeepSkyBlue") difference() {
            ...
            // 4 ГНЕЗДА ПОД ПИРАМИДКИ (перевёрнутые пирамидки, сужаются книзу)
            top_size = pyramid_size * 0.6;
            
            // Передние гнезда (для первой отливки)
            translate([pyramid_offset + pyramid_size/2, 
                       pyramid_offset + pyramid_size/2, 
                       bottom_case_thickness + 0.1])
                inverted_pyramid(pyramid_size, top_size, pyramid_height);
            
            translate([fixture_width - pyramid_offset - pyramid_size/2, 
                       pyramid_offset + pyramid_size/2, 
                       bottom_case_thickness + 0.1])
                inverted_pyramid(pyramid_size, top_size, pyramid_height);
            
            // Задние гнезда (для второй отливки)
            translate([pyramid_offset + pyramid_size/2, 
                       total_depth - pyramid_offset - pyramid_size/2, 
                       bottom_case_thickness + 0.1])
                inverted_pyramid(pyramid_size, top_size, pyramid_height);
...
// ═══════════════════════════════════════════════════════════════════
// РЕЖИМ ASSEMBLY - СБОРКА ВНУТРИ ОПАЛУБКИ
// ═══════════════════════════════════════════════════════════════════

else if (view_mode == "assembly") {
    // Параметры прозрачности
    formwork_alpha = 0.3;      // Прозрачность опалубки
    master_alpha = 0.5;        // Прозрачность мастер-модели
    plate_alpha = 1.0;         // Непрозрачная пластина
    
    // ОПАЛУБКА (полупрозрачная)
    color("Red", formwork_alpha) 
        formwork(p_d, p_cl, p_th, formwork_height, outer_x_w, formwork_y_d);
    
    // МАСТЕР-ПЛАСТИНА (внутри опалубки)
    translate([0, 0, 0]) {
        color("LightGreen", plate_alpha) 
            master_plate(p_d, p_cl, yellow_plate_thickness, h_t, true);
        
        // Шип
        translate([fixture_width/2, 0, 0]) 
            color("Lime", plate_alpha) 
                dovetail_rail_vertical(width = dovetail_width, depth = dovetail_depth, height = dovetail_height, angle = dovetail_angle);
    }
    
    // МАСТЕР-МОДЕЛЬ (прозрачная, надета на пластину)
    translate([0, 0, 0]) {
        color("Orange", master_alpha) 
            difference() {
                intersection() {
                ...
                translate([0, 0, -1]) 
                    mirror([0, 1, 0])
                    color("DarkOrange") 
                        dovetail_rail_vertical(width = dovetail_width + dovetail_gap, depth = dovetail_depth, height = dovetail_height + 1, angle = dovetail_angle);
            }
    }
    
    // Опционально: калибровочные точки (если включены)
    if (show_calibration_points) {
        ...
        // Точки на перевёрнутых пирамидках в синей детали (центр каждой)
        // Передняя левая
        color("Red") translate([pyramid_offset + pyramid_size/2, 
                                pyramid_offset + pyramid_size/2, 
                                bottom_case_thickness + 0.1]) sphere(r = 1.5);
        // Передняя правая
        color("Green") translate([fixture_width - pyramid_offset - pyramid_size/2, 
                                  pyramid_offset + pyramid_size/2, 
                                  bottom_case_thickness + 0.1]) sphere(r = 1.5);
        // Задняя левая
        color("Blue") translate([pyramid_offset + pyramid_size/2, 
                                 total_depth - pyramid_offset - pyramid_size/2, 
                                 bottom_case_thickness + 0.1]) sphere(r = 1.5);
        // Задняя правая
        color("Yellow") translate([fixture_width - pyramid_offset - pyramid_size/2, 
                                   total_depth - pyramid_offset - pyramid_size/2, 
                                   bottom_case_thickness + 0.1]) sphere(r = 1.5);
    }
    
    // Оси и маркеры (если включены)
    if (show_axes) {
    ...
    // Разрез, если включен
    if (cut_view) {
        color("Gray", 0.3) translate([-200, -200, -eps]) cube([400, 400, h_t + 100]);
    }
}
...
module master_plate(pd, pcl, pth, ht, use_extra=false) {
    ...
    // Сферы остаются на оригинальной высоте (ht_original = h_t)
    // А сама пластина выше на master_plate_extra_height
    ht_original = h_t;
    
    difference() {
        union() {
            // Пластина увеличенной высоты
            cube([fixture_width, pth, master_plate_height]);
            
            // Сферы на оригинальной высоте (от низа пластины) - только если show_spheres включен
            if (show_spheres) {
                translate([off, +comp, off]) sphere(r = r_s);
...
translate([-800, 0, 0]) {
    ...
    // Калибровочные точки паза (только если включены)
    if (show_calibration_points) {
        color("Red") translate([-(dovetail_width + dovetail_gap)/2, 0, dovetail_height]) sphere(r = 1.5);
        color("Green") translate([+(dovetail_width + dovetail_gap)/2, 0, dovetail_height]) sphere(r = 1.5);
        color("Blue") translate([-(dovetail_width + dovetail_gap)/2, 0, 0]) sphere(r = 1.5);
    ...
    // Оси и маркеры (только если show_axes = true)
    if (show_axes) {
        translate([0, 0, dovetail_height / 2]) {
            color("Yellow") sphere(r = 2);
            color("Red")   translate([-15, 0, 0]) cube([30, 0.5, 0.5]);
...
// МАСТЕР-ПЛАСТИНА С ШИПОМ (увеличенной высоты, сферы на оригинальной высоте)
translate([-450, 0, 0]) {
    ...
    // Калибровочные точки шипа (только если включены)
    if (show_calibration_points) {
        color("Red") translate([fixture_width/2 - dovetail_width/2, 0, dovetail_height]) sphere(r = 1.5);
        color("Green") translate([fixture_width/2 + dovetail_width/2, 0, dovetail_height]) sphere(r = 1.5);
        color("Blue") translate([fixture_width/2 - dovetail_width/2, 0, 0]) sphere(r = 1.5);
