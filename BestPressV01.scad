// =============================================================
// ПРЕСС-ФОРМА ДЛЯ КЕРАМИЧЕСКОЙ ПЛИТКИ (V11.4 - CLEAN TOP HANDLE)
// =============================================================
...
module crossbar() {
    ...
    difference() {
        union() {
            minkowski() {
                hull() {
                    translate([0, 0, (bar_h - 2)/2]) cube([main_w - 4, outer_size - 4, bar_h - 2], center=true);
                    for(i=[-1,1]) translate([0, i * (bw/2 - 5), bar_h - 3]) cylinder(r=main_w/2 - 2, h=1, center=true);
                }
                sphere(r=2, $fn=32);
            }
            for(i=[-1,1]) translate([0, i*(inner_size/2 + (wall_thick - actual_slot_depth)/2), bar_slot_h/2])
                cube([main_w, (wall_thick - actual_slot_depth), bar_slot_h], center=true);
        }
        translate([0, 0, -5.1]) cube([main_w + 15, bw + 15, 10], center=true);
        for(i=[-1,1]) translate([0, i*dist/2, -0.5]) cylinder(h = bar_h + 2, d = rod_d + comp + play, $fn=64);
        translate([0, 0, -0.1]) cylinder(h = bar_h - 2.1, d = cross_L + 2, $fn=64); 
    }
}
...
module handle() {
    ...
    difference() {
        union() {
            minkowski() {
                hull() { 
                    translate([0,0,0]) cube([18-2*r_s, h_l-2*r_s, 0.1], center=true); 
                    translate([0,0,10]) rotate([0,90,0]) scale([1, h_l/18, 1]) cylinder(h=18-2*r_s, d=18-2*r_s, center=true); 
                }
                sphere(r=r_s, $fn=16);
            }
            for(i=[-1,1]) translate([0, i*dist/2, 0]) cylinder(h=1.5, d=rod_d + 4, $fn=32);
        }
        // ИСПРАВЛЕНО: Отверстия начинаются снизу и НЕ пробивают верх (глубина 15мм)
        for(i=[-1,1]) translate([0, i*dist/2, -5]) 
            cylinder(h=15, d=rod_d + comp, $fn=32);
    }
}
...
module d_rod() {
    rotate([0,0,0]) intersection() {
        ...
        translate([0, 0.8, 0]) cube([rod_d, rod_d, 120], center=true);
    }
}
...
if (view_mode == "assembly") {
    ...
    intersection() { plunger_plate(); cube([30,30,30], center = true); }
    translate([40,0,0]) intersection() { plunger_cap(extra_off = cross_test_offset); cube([30,30,30], center = true); }
    ...
    for(i=[-1,1]) translate([i*25, outer_size + 80, 0]) rotate([0,-90,0]) d_rod();
} else {
    ...
    if(view_mode=="rods") for(i=[-1,1]) translate([i*25, 0, 0]) rotate([0,-90,0]) d_rod();
}
