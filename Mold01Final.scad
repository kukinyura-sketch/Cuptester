// Slip-Cast Hybrid Generator v15.4
// Генератор форм для шликерного литья
// ПОЛНАЯ ВЕРСИЯ: без ребра, цельная задняя стенка 1.6мм, пирамидки у передней стенки
...
fixture_width = 160; // [80:10:300] Ширина монтажной пластины (мм)
...
pyramid_offset = 20; // [10:1:50] Отступ пирамидки от края и передней стенки (мм)
...
show_axes = true;           // [true:ПОКАЗАТЬ ОСИ И МАРКЕРЫ, false:СКРЫТЬ]
...
yellow_plate_start = front_wall_thickness + inner_frame_depth; // 3 + 80 = 83 мм
yellow_plate_end = yellow_plate_start + yellow_plate_thickness; // 83 + 15 = 98 мм

// Координата для фиксирующих блоков (там же, где пластина)
yellow_plate_block_y = yellow_plate_start; // 83 мм

// Положение пирамидок (в полости, у передней стенки)
pyramids_y = front_wall_thickness + pyramid_offset; // 3 + 20 = 23 мм
...
// ═══════════════════════════════════════════════════════════════════
// ОПАЛУБКА (красная рамка, без ребра, цельная задняя стенка 1.6мм, пирамидки у передней стенки)
// ═══════════════════════════════════════════════════════════════════
...
if (view_mode == "all") {
    ...
    // СТАНЦИЯ 2: Желтая пластина с шариками - СКРЫТА (не отображается, но координаты станции сохранены)
    // translate([300, 0, 0]) 
    //     color("Yellow") plate_with_spheres(p_d, p_cl, p_th, h_t, true, cup_shape);

    // СТАНЦИЯ 3: ОПАЛУБКА (красная)
    translate([600, 0, 0]) 
        color("Red") formwork(p_d, p_cl, p_th, h_t, outer_x_w, formwork_y_d);
    
    // СТАНЦИЯ 4: ОПАЛУБКА ДНА (синяя нижняя форма, 4 гнезда под пирамидки - спереди и сзади)
    translate([900, -total_depth/2, 0]) 
        color("DeepSkyBlue") difference() {
            union() {
                difference() {
                    cube([outer_x_w, total_depth, 30 + (bottom_case_thickness - front_wall_thickness)]); 
                    translate([front_wall_thickness, front_wall_thickness, bottom_case_thickness]) 
                        cube([fixture_width, total_depth - front_wall_thickness*2, 40]); 
            ...
            // 4 ГНЕЗДА ПОД ПИРАМИДКИ (2 спереди + 2 сзади)
            // Передние гнезда (для первой отливки)
            translate([front_wall_thickness + pyramid_offset, front_wall_thickness + pyramid_offset, bottom_case_thickness - pyramid_height])
                cube([pyramid_size, pyramid_size, pyramid_height + 1]);
            translate([front_wall_thickness + fixture_width - pyramid_offset - pyramid_size, front_wall_thickness + pyramid_offset, bottom_case_thickness - pyramid_height])
                cube([pyramid_size, pyramid_size, pyramid_height + 1]);
            // Задние гнезда (для второй отливки) - отступ от задней стенки
            translate([front_wall_thickness + pyramid_offset, total_depth - back_wall_thickness - pyramid_offset - pyramid_size, bottom_case_thickness - pyramid_height])
                cube([pyramid_size, pyramid_size, pyramid_height + 1]);
            translate([front_wall_thickness + fixture_width - pyramid_offset - pyramid_size, total_depth - back_wall_thickness - pyramid_offset - pyramid_size, bottom_case_thickness - pyramid_height])
                cube([pyramid_size, pyramid_size, pyramid_height + 1]);
        }
}
...
// ═══════════════════════════════════════════════════════════════════
    ...
    color("Goldenrod", 0.6) render_body(total_height, false, cup_shape);
    ...
    if (cut_view) {
        color("Gray", 0.3) translate([-200, -200, -eps]) cube([400, 200, total_height + 50]);
        for (z = [0:2:total_height]) {
            let(r = get_radius(z / total_height, cup_shape))
            color("Red", 0.2) translate([0, 0, z]) cylinder(r = r, h = 0.2);
        }
    }
}
...
// ═══════════════════════════════════════════════════════════════════
// ГЛАВНАЯ ОСНАСТКА: Ласточкин хвост (Мастер-модель + Мастер-пластина)
// ═══════════════════════════════════════════════════════════════════
...
module dovetail_rail_vertical(width, depth, height, angle) {
    // Строим от Z=0 вверх (без центрирования)
    // Широкая часть в положительном Y, узкая в отрицательном Y
...
module master_plate(pd, pcl, pth, ht, use_extra=false) {
    ...
    difference() {
        union() {
            ...
            translate([fixture_width - off, -comp, ht - off]) sphere(r = r_s);
        }
        translate([0, 0, -eps_global]) pins_lock(pd + pcl, pcl, ht, true);
        translate([fixture_width, 0, -eps_global]) pins_lock(pd + pcl, pcl, ht, true);
        if (use_extra) {
            translate([fixture_width - off, -comp, off]) sphere(r = r_s + 0.3);
            translate([fixture_width - off, -comp, ht - off]) sphere(r = r_s + 0.3);
        }
...
// МАСТЕР-МОДЕЛЬ С ПАЗОМ (обрезанная, паз внутри модели)
translate([-800, 0, 0]) {
    // Принудительный рендер для гарантии экспорта паза
    render() difference() {
        // Обрезанная мастер-модель
        ...
        // Паз строится от Z=-1 (гарантированно прорезает дно) вверх
        // Ориентация паза: широкая часть внутри модели (Y положительный), узкая у стенки (Y отрицательный)
        translate([0, 0, -1]) 
            mirror([0, 1, 0])  // Зеркалим по Y для правильной ориентации
    ...
    // 4 угловые точки паза (разных цветов) для визуального контроля
    // Верхние точки (Z = dovetail_height)
    ...
    // Оси и маркер для проверки положения паза (только если show_axes = true)
    if (show_axes) {
        // Маркер в центре паза (по X,Y центр модели, Z = середина высоты паза)
        translate([0, 0, dovetail_height / 2]) {
            // Желтая сфера-маркер в центре паза
            color("Yellow") sphere(r = 2);
            
            // Оси координат от центра паза
            color("Red")   translate([-15, 0, 0]) cube([30, 0.5, 0.5]); // ось X
            color("Green") translate([0, -15, 0]) cube([0.5, 30, 0.5]); // ось Y
            color("Blue")  translate([0, 0, -15]) cube([0.5, 0.5, 30]); // ось Z
        }
        
        // Дополнительная метка в основании паза (Z=0)
        translate([0, 0, 0]) 
            color("Cyan") sphere(r = 1.5);
        
        // Метка в верхней точке паза (Z = dovetail_height)
        translate([0, 0, dovetail_height]) 
            color("Magenta") sphere(r = 1.5);
    }
}

// МАСТЕР-ПЛАСТИНА С ШИПОМ (шип от Z=0 вверх, высота пластины = h_t)
translate([-450, 0, 0]) {
    color("LightGreen") master_plate(p_d, p_cl, yellow_plate_thickness, h_t, true);
    // Шип строится от Z=0 (низ пластины) вверх
    ...
    // 4 угловые точки шипа (разных цветов) для визуального контроля
    // Верхние точки (Z = dovetail_height)
    ...
    // Оси и маркер для проверки положения шипа (только если show_axes = true)
    if (show_axes) {
        // Маркер в центре шипа
        translate([fixture_width/2, 0, dovetail_height / 2]) {
            // Желтая сфера-маркер в центре шипа
            color("Yellow") sphere(r = 2);
            
            // Оси координат от центра шипа
            color("Red")   translate([-15, 0, 0]) cube([30, 0.5, 0.5]); // ось X
            color("Green") translate([0, -15, 0]) cube([0.5, 30, 0.5]); // ось Y
            color("Blue")  translate([0, 0, -15]) cube([0.5, 0.5, 30]); // ось Z
        }
        
        // Дополнительная метка в основании шипа (Z=0)
        translate([fixture_width/2, 0, 0]) 
            color("Cyan") sphere(r = 1.5);
        
        // Метка в верхней точке шипа (Z = dovetail_height)
        translate([fixture_width/2, 0, dovetail_height]) 
            color("Magenta") sphere(r = 1.5);
    }
}
