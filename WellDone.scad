// Slip-Cast Hybrid Generator v15.4 - ПАРАМЕТРИЧЕСКАЯ ВЕРСИЯ
// FIX: Центрирование ласточкина хвоста
...
manual_height = 0; // [0:50:200] 0 = использовать базовый
...
bottom_radius = 3; // [0:1:15] Скругление дна (мм)
bottom_concave = 0.5; // [0:0.5:8] Вогнутость дна (мм)
...
module green_frame(pd, pcl, pth, ht, gw, xw, yd) {
    ...
    union() {
        difference() {
            cube([xw, yd, ht]);
            translate([gw, gw, gw]) cube([fixture_width, yd + 1, ht + 1]);
        }
        ...
        color("DeepSkyBlue") difference() {
            union() {
                difference() {
                    ...
                    translate([gw, gw, bottom_case_thickness]) cube([fixture_width, total_depth - gw*2, 40]); 
...
// ═══════════════════════════════════════════════════════════════════
// МОДУЛЬ 1: ОРАНЖЕВАЯ МАСТЕР-МОДЕЛЬ (СЛЕВА)
// ═══════════════════════════════════════════════════════════════════
...
translate([-450, 0, 0]) {
    ...
    translate([fixture_width/2, 0, h_t/2])
        color("LightGreen")
            dovetail_rail_vertical(
                width = 15,
                depth = 10,
